...
- 2026-02-15: SSE scan stream API tests should bound long-lived streams with a `max_events` query limit when using `httpx.ASGITransport`; backend (`ruff` + `pytest`) and frontend (`tsc` + `build`) backpressure checks pass after enabling live scan stream updates with polling fallback.
- 2026-02-15: Auth/telegram edge-case coverage can be expanded safely with fake Telethon error injection (`RPCError` paths, missing `phone_code_hash`, non-authorizing `sign_in`) plus direct schema validation tests, without any live Telegram/network access.
